...
python -m pip install --no-cache-dir --only-binary :all: protobuf tqdm typing-extensions requests 2>/dev/null

# Install pydantic (might need --no-binary pydantic-core if crashes)
echo "Installing pydantic..."
python -m pip install --no-cache-dir pydantic 2>/dev/null || python -m pip install --no-cache-dir --only-binary :all: pydantic 2>/dev/null

# Install google packages without heavy dependencies
echo "Installing Google packages (minimal)..."
python -m pip install --no-cache-dir --only-binary :all: googleapis-common-protos proto-plus 2>/dev/null
python -m pip install --no-cache-dir --only-binary :all: google-auth 2>/dev/null

# Install google-api-core WITHOUT grpc extras
echo "Installing google-api-core (REST only, no gRPC)..."
python -m pip install --no-cache-dir --only-binary :all: google-api-core 2>/dev/null

# Install the generative AI packages without dependencies first
echo "Installing google-ai-generativelanguage..."
python -m pip install --no-cache-dir --no-deps google-ai-generativelanguage 2>/dev/null

echo "Installing google-api-python-client..."
python -m pip install --no-cache-dir --only-binary :all: google-api-python-client 2>/dev/null

echo "Installing google-generativeai (no deps)..."
python -m pip install --no-cache-dir --no-deps google-generativeai 2>/dev/null
...
if python3 -c "import google.generativeai; import PIL; print('✅ All imports working!')" 2>/dev/null; then
    ...
    python -m pip install --no-cache-dir googleapis-common-protos proto-plus google-auth
